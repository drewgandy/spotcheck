...
#con = mdb.connect('localhost', 'root', 'password', 'CA_UCC_testdb');
...
def create_dictionary_entry(w):
    ...
    for item in deletes:
        ...
        with con:
            cur = con.cursor()
            cur.execute("INSERT INTO UCCDictionary(Name, OriginalName) VALUES(%s, %s);", (item, w))
...
def checkspell():
    ...
    current_time = 0
#   try:
    f = open("/Users/laurabeth/Desktop/SpotCheck/CA/Corp/CompanyNames.txt", 'r')
    ...
    var2 = StringVar()
#var3 = StringVar()
    label = Label( root, textvariable=var, relief=RAISED, justify = LEFT, width = 300 )
    label2 = Label( root, textvariable=var2, relief=RAISED, justify = LEFT )
#label3 = Label( root, textvariable=var3, relief=RAISED, justify = LEFT )
...
#label3.pack()
    ...
    for line in f:
        ...
        var.set(line)
        var2.set(str((linecount / 196388)*100)+"%")
        #var3.set(((5400000-linecount)/(linecount/time.time()))/60/60)
        label.pack()
        label2.pack()
        #label3.pack()
        root.update_idletasks()
        root.update()
     #create_dictionary("/Users/laurabeth/Desktop/SpotCheck/CA/Corp/CompanyNames.txt")
        create_dictionary_entry(line);
    f.close()
    #except:
...
def checkspell_DB():
    ...
    with con:
        cur = con.cursor()
        cur.execute("DROP TABLE IF EXISTS UCCDictionary")
        cur.execute("CREATE TABLE UCCDictionary(Id INT PRIMARY KEY AUTO_INCREMENT, Name VARCHAR(300), OriginalName VARCHAR(300))")
    ...
    current_time = 0
    #   try:
    f = open("/Users/laurabeth/Desktop/SpotCheck/CA/UCC/UCCDict.txt", 'r')
    print("OPEN: " + str(time.clock()))
    linecount = 0.0
    from Tkinter import *
    ...
    for line in f:
        ...
        var.set(line)
        var2.set(str((linecount / 1102507)*100)+"%") #5400000
        #var3.set(((5400000-linecount)/(linecount/time.time()))/60/60)
        label.pack()
        label2.pack()
        #label3.pack()
